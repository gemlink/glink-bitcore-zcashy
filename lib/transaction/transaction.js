...
var DEFAULT_VALUEBALANCE = new BN(0);
...
Transaction.prototype.setBranchId = function(id){
  this.branchId = id;
}
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.to = function(address, amount, network) {
  if (_.isArray(address)) {
    var self = this;
    _.each(address, function(to) {
      self.to(to.address, to.satoshis, network);
    });
    return this;
  }
  ...
  this.addOutput(new Output({
    script: Script(new Address(address, network)),
    satoshis: amount
  }));
  return this;
};
